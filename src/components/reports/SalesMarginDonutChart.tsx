...
import type { SalesMarginByCategory } from '@/types/odoo.types';
import { useResponsive } from '@/hooks/useResponsive';
import { useTheme } from '@/theme/ThemeContext';

interface Props {
  data: SalesMarginByCategory[];
  title?: string;
}

export default function SalesMarginDonutChart({
  data,
  title = 'Margin by Category',
}: Props) {
  const { width, contentPadding, isTablet } = useResponsive();
  const availableSpace = width - contentPadding * 2;
  ...
  const victoryData = data.map((d) => ({
    ...
    y: d.marginPercent,
    color: d.color,
    label: `${d.marginPercent}%`,
  }));

  const totalMarginPercent = data.reduce((sum, d) => sum + d.marginPercent, 0);
  ...
  const avgMarginPercent =
    data.length > 0 ? Math.round(totalMargin / data.reduce((sum, d) => sum + d.revenue, 0) * 100) : 0;
      ...
      <View style={styles.legend}>
        {data.map((d) => (
          <View key={d.id} style={styles.legendItem}>
            ...
            <Text style={[styles.legendCategory, { color: colors.textSecondary }]}>
              {d.category}
            </Text>
